...
class Ip_Tabmegamenu_Admin_CategoryController extends Mage_Adminhtml_Controller_Action {
    ...
    public function indexAction() {
        $this->_initAction()
                ->_addContent($this->getLayout()->createBlock('tabmegamenu/admin_category'))
                ->renderLayout();
    }
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('tabmegamenu/category');
        // 2. Initial checking
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('tabmegamenu')->__('This category no longer exists'));
                $this->_redirect('*/*/');
                return;
            }
        }
        ...
        Mage::register('tabmegamenu_category', $model);
        // 5. Build edit form
        $this->_initAction()
                ...
                ->_addContent($this->getLayout()->createBlock('tabmegamenu/admin_category_edit')->setData('action', $this->getUrl('*/admin_category/save')))
                ->_addLeft($this->getLayout()->createBlock('tabmegamenu/admin_category_edit_tabs'))
                ->renderLayout();
    }
...
//            print_r($data);exit();
    ...
    public function deleteAction() {
        ...
        // display error message
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('tabmegamenu')->__('Unable to find a category to delete'));
        // go to grid
        $this->_redirect('*/*/');
    }
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('tabmegamenu/category');
    }
        ...
        if(!is_array($categoryIds)) {
            ...
            try {
                foreach ($categoryIds as $categoryId) {
                    $model = Mage::getSingleton('tabmegamenu/category')
                        ->load($categoryId)
   ...
   public function massDeleteAction() {
        $categoryIds = $this->getRequest()->getParam('massaction');
        if(!is_array($categoryIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('tabmegamenu')->__('Please select category(s)'));
        } else {
            try {
                foreach ($categoryIds as $categoryId) {
                    $mass = Mage::getModel('tabmegamenu/category')->load($categoryId);
                    $mass->delete();
                }
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('tabmegamenu/admin_category_grid')
            ->getCsv();
